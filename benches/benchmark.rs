...
fn bench(c: &mut Criterion, name: &str, bytes: &[u8]) {
    ...
    for i in [1, 8, 64, 512].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("simd", slice.len()),
            &slice,
            |b, &slice| {
                b.iter(|| validate_utf8(slice).unwrap());
            },
        );
        group.bench_with_input(BenchmarkId::new("std", slice.len()), &slice, |b, &slice| {
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    bench(c, "latin", include_str!("text/Latin-Lipsum.txt").as_bytes());
    bench(
        c,
        "cyrillic",
