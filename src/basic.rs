//! The `basic` API flavor provides barebones UTF-8 checking at the highest
//! speed.
//!
//! It is fastest on valid UTF-8, but only checks for errors after processing
//! the whole byte sequence and does not provide detailed information if the
//! data is not valid UTF-8. [`Utf8Error`] is a zero-sized error struct.
//!
//! If you need detailed error information use the functions from the
//! [`crate::compat`] module instead.
...
///
/// No information is provided where the error occured or how long the invalid
/// byte byte sequence is.
...
pub mod imp {
    ...
    /// A low-level interfacne for streaming validation of UTF-8 data. It is
    /// meant to be integrated in high-performance data processing
    /// pipelines.
    ///
    /// Data can be streamed in arbitrarily-sized chunks using the
    /// [`Self::update()`] method. There is no way to find out if the input
    /// so far was valid UTF-8 during the validation. Only when
    /// the validation is completed with the [`Self::finalize()`] method the
    /// result of the validation is returned. Use [`ChunkedUtf8Validator`]
    /// if possible for highest performance.
    ///
    /// This implementation requires CPU SIMD features specified by the module
    /// it resides in. It is undefined behavior to use it if the required
    /// CPU features are not available which is why all trait methods are
    /// `unsafe`.
    ///
    /// General usage:
    /// ```rust
    ...
    pub trait Utf8Validator {
        ...
        ///
        /// # Safety
        /// This implementation requires CPU SIMD features specified by the
        /// module it resides in. It is undefined behavior to call it if
        /// the required CPU features are not available.
        #[must_use]
        unsafe fn new() -> Self
        where
            Self: Sized;
        ...
        /// Finishes the validation and returns `Ok(())` if the input was valid
        /// UTF-8.
        ///
        /// # Errors
        /// A [`basic::Utf8Error`] is returned if the input was not valid UTF-8.
        /// No further information about the location of the error is
        /// provided.
        ///
        /// # Safety
        /// This implementation requires CPU SIMD features specified by the
    ...
    /// Like [`Utf8Validator`] this low-level API is for streaming validation of
    /// UTF-8 data. It has additional restrictions imposed on how the input
    /// is passed in to allow validation with as little overhead as
    /// possible.
    ///
    /// To feed it data you need to call the [`Self::update_from_chunks()`]
    /// method which takes slices which have to be a multiple of 64 bytes
    /// long. The method will panic otherwise.  There is no way to find out
    /// if the input so far was valid UTF-8 during the validation. Only when
    /// the validation is completed with the [`Self::finalize()`] method the
    /// result of the validation is returned.
    ///
    /// The `Self::finalize()` method can be fed the rest of the data. There is
    /// no restriction on the data passed to it.
    ///
    /// This implementation requires CPU SIMD features specified by the module
    /// it resides in. It is undefined behavior to use it if the required
    /// CPU features are not available which is why all trait methods are
    ...
    pub trait ChunkedUtf8Validator {
        ...
        /// Updates the validator with remaining input if any. There is no
        /// restriction on the data provided.
        ///
        /// Finishes the validation and returns `Ok(())` if the input was valid
        /// UTF-8.
        ///
    ...
    pub mod x86 {
        ...
        ///
        /// Using the provided functionality on CPUs which do not support AVX 2
        /// is undefined behavior and will very likely cause a crash.
        pub mod avx2 {
        ...
        ///
        /// Using the provided functionality on CPUs which do not support SSE
        /// 4.2 is undefined behavior and will very likely cause a
        /// crash.
        pub mod sse42 {
    ...
    pub mod aarch64 {
        ...
        ///
        /// Should be supported on all ARM64 CPUSs. If it is not supported by
        /// the operating system using it is undefined behavior and will
    ...
    pub mod wasm32 {
        /// Includes the simd128-based validation implementation for WASM
        /// runtimes.
        ///
        /// Using the provided functionality on WASM runtimes that do not
        /// support SIMD instructions will likely cause a crash.
        pub mod simd128 {
